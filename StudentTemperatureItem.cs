...
namespace BodyTemperature
{
    ...
    [FISCA.Permission.FeatureCode("BodyTemperature.Student.Item", "學生體溫記錄")]
    internal partial class StudentTemperatureItem : DetailContentBase
    {
        BackgroundWorker BGW = new BackgroundWorker();
        bool BkWBool = false;
        private List<BodyTmperature> _records = new List<BodyTmperature>();
        StudentRecord student;
        public StudentTemperatureItem()
        {
            InitializeComponent();
        ...
        private void BGW_DoWork(object sender, DoWorkEventArgs e)
        {
            if (this.PrimaryKey != "")
            {
                student = K12.Data.Student.SelectByID(this.PrimaryKey);

                _records.Clear();
                _records = tool._A.Select<BodyTmperature>(string.Format("body_tag='Student' and ref_obj_id='{0}'", this.PrimaryKey));
            }
        }
        ...
        {
            EditTmperatureForm editor = new EditTmperatureForm(tool.BodyState.Student, int.Parse(this.PrimaryKey));
            editor.ShowDialog();
        }
        ...
        private void btnEdit_Click(object sender, EventArgs e)
        {
            ...
            {
                EditTmperatureForm editor = new EditTmperatureForm(tool.BodyState.Student, int.Parse(this.PrimaryKey), (BodyTmperature)listView.SelectedItems[0].Tag);
                editor.ShowDialog();
            }
            else
            {
